# ParityLens — Operating Contract

## Mission

Deliver ParityLens, a VS Code extension for profiling, reconciling, and
validating datasets across heterogeneous data platforms, for data engineers,
analytics engineers, and migration teams who need to prove parity between a
source and target platform (e.g. SQL Server → Snowflake, Oracle → Fabric)
without leaving their development workflow. The completed result satisfies
the approved design, preserves the stated safety boundaries, and can be
...
- `DESIGN-SPEC.md` — approved scope, technical decision, and acceptance criteria (not yet created).
- `IMPLEMENTATION-PLAN.md` — dependency order, interfaces, ownership, and gates (not yet created).
- `PROGRESS-LEDGER.md` — current lifecycle state, active task, decisions, and blockers.
- `TASK-BRIEF.md`, `IMPLEMENTATION-REPORT.md`, and `REVIEW-REPORT.md` — task evidence (created per task, once implementation starts).
- `RELEASE-CHECKLIST.md` — release evidence and final approval (not yet created).
- `multi-agent-idea-to-app/` — the lifecycle kit itself: prompts, templates, and adapters referenced by phase.
- `Idea Prompt.md` — the original product concept document this project is built from.

When two control files disagree, stop work, record the conflict in the ledger,
and obtain the decision maker's written decision before changing scope.
...
- Treat all source and target database connections as read-only unless a
  recorded approval says otherwise. ParityLens must never write back to a
  source or target system under comparison.
- Do not delete, overwrite, move, publish, send, deploy, charge, or contact
  external parties without the approval recorded in `PROJECT-BRIEF.md`.
- Keep secrets, personal data, production credentials, and unapproved
  customer data out of logs, reports, tests, and generated artifacts.
  Credentials must never be stored in parity configuration files — use VS
  Code SecretStorage, environment variables, or native cloud/OS credential
  mechanisms only.
- Use isolated output paths under a safe output root (e.g. a project-local
  `work/` or `.paritylens/` directory); verify containment before writing.
- Block INSERT, UPDATE, DELETE, DROP, and other mutating statements from
  being executed against connected source or target systems.
- Preserve unrelated changes. Never use destructive repository commands
  unless the decision maker explicitly authorizes the exact target and
  recovery plan.
