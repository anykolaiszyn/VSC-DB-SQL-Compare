...
- `packages/extension/package.json` (`private` field removal, `name`,
  `publisher`, plus a new `scripts.package` entry — do not touch any
  other field)
...
**Amendment (round 1 continuation, added after the first implementer
correctly stopped and flagged an ownership boundary rather than
self-authorizing an edit):** `@vscode/vsce@3.9.2`'s actual, first-
encountered blocker is not `private: true` (that field is not checked by
this `vsce` version at all, per the first implementer's source-level
investigation, already correct in the report) but two required manifest
fields neither present nor previously in scope: `name` (currently
`@paritylens/extension`, which is invalid — VS Code extension manifest
names cannot contain `@`/`/`, only `/^[a-z0-9][a-z0-9\-]*$/i`) and
`publisher` (currently absent entirely). Owner confirmed directly when
asked: set `"name": "paritylens"` (bare, no scope prefix — matches the
product name used everywhere else in this project) and
`"publisher": "parity-lens-dev"` (an explicit placeholder — no real VS
Code Marketplace publisher account exists yet; this produces a real,
valid, installable `.vsix` for local/internal use without requiring
Marketplace registration right now). Disclose this placeholder-publisher
status plainly in both `IMPLEMENTATION-REPORT.md` and the new
`README.md` (a one-line note is sufficient — e.g. "published under a
placeholder publisher ID pending real Marketplace registration").
