...
- **Patch or commit:** `8262f06ce6d779161b4afdc6f659645cce5c1233` (commit
  message: "T-48: add source→target segment to results webview header
  meta-line"). Note: this hash was generated by committing the 5 owned
  files together with an earlier draft of this report; the report content
  itself (this file) was subsequently finalized with the real hash filled
  in, so this specific line was written after the commit that produced the
  hash it names, per `git log -1` at the time of writing.
